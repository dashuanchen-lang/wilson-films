...
function sellingGrid(videos){return `<div class="selling-grid">${videos.map(v=>`<article class="selling-work">${videoTile(v,v.id==='advertising'?'剧情广告':sellingLabels[v.id])}<a class="selling-case-link" href="${v.id==='advertising'?'#/project/advertising':'#/contact'}">${v.id==='advertising'?'个人参赛作品 · 非品牌委托':'沟通同类视频制作'} ${arrow}</a></article>`).join('')}</div>`;}
